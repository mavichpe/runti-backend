...
class PlaylistsController extends AppController {
    ...
    public function getPlaylists() {
        ...
        $this->Playlist->recursive = 0;
        $playlists = $this->Playlist->find('all', array('conditions' => 'Playlist.habilitado = true'));
        $this->response->body(json_encode($playlists));
    }

    public function getPlaylist($id) {
        $this->autoRender = false;
        $this->Playlist->recursive = 0;
        $playlist = $this->Playlist->find('first', array('conditions' => 'Playlist.id = ' . $id));
